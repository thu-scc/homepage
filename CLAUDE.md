...
- Two accent families, both gold. `--accent*` is the interactive accent in three tiers: `--accent` for fills (primary button, pressed states, with `--on-accent` text), `--accent-strong` for icons, focus rings and large emphasis, `--accent-text` / `--accent-text-2` for small text and hovers. In dark all tiers are the same gold; in light the fill stays bright (`#d9b25a`) while the text tier darkens to `#8a6a12` so it passes 4.5:1. Never put `--accent` on small text. `--gold*` marks a championship and nothing else: chips (`--gold-soft` fill, `--gold-ink` text, `--gold-line` border), timeline dots and grand-slam rows. Every other result is a neutral outline; do not add per-result-type colours back. Photo scrims use `--scrim-rgb`. The primary button class is `.btn-primary`.
...
- Motion dial is 6 and every piece is motivated: Astro's `ClientRouter` gives a short cross-page fade, and a photo card's `transition:name="photo-<id>"` morphs into the matching event opener; hero photographs settle from a slight zoom; below-the-fold sections fade up once (`data-reveal`) with children of a `.stagger` container following in sequence (`--i` set inline); the championship timeline draws its rail and dots in year order; the overlay header becomes solid over the first 320px of scrolling via a CSS scroll-driven animation (fixed only where supported); record figures with `data-count` count up; FAQ answers, the mobile menu and the member dialog have open and close transitions. All of it collapses to static under `prefers-reduced-motion`. No looping animation, no scroll listeners, no gradients other than photo scrims, no glass. The one blur is the masked, pre-rasterised copy of a hero photograph behind its text (`img.hero-blur`), which the owner asked for; no `backdrop-filter` anywhere.
