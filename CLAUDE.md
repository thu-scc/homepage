...
- Plum and gold in both themes: a plum ground with ivory text, or a violet-tinted ivory ground with plum ink and a deeper gold (`--gold` is `#7f6115` in light so small gold text still passes 4.5:1). Gold is the single accent: championship chips and the one primary action per page. Every other result is a neutral outline; do not add per-result-type colours back. Text on gold uses `--on-gold`; photo scrims use `--scrim-rgb`.
